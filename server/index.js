...
app.post( '/api/auth/register', userController.regester ); //Registers a user to the database. Sets the user information on the session. On success return a status of 200 and the user object {id, username}. On failure return a status of 500.
app.post( '/api/auth/logout', userController.logout ); //Destroys the session. Sends a status of 200.

//Property API commands:
app.post( '/api/properties', propController.createNew); //Creates a new property. Returns all the properties associated with the logged in user.
app.get( '/api/properties', propController.getProps ); //Returns all properties associated with the logged in user. Accepts an optional query that filters all properties associated with the logged in user by "desired rent". Returns all the filtered properties.
app.delete( '/api/properties/:id', propController.deleteProp ); //Deletes a property. Returns all the properties associated with the logged in user.
